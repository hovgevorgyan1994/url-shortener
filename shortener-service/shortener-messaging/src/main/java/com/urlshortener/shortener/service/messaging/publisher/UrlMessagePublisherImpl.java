...
public class UrlMessagePublisherImpl implements UrlMessagePublisher {
    ...
    private final KafkaProducer<String, ShortenRequestAvroModel> producer;
    ...
    public Mono<Void> publish(UrlShortenedEvent urlShortenedEvent) {
        return Mono.just(urlShortenedEvent)
            .doOnNext(event -> {
                var avroModel = requestMapper.eventToAvroModel(event);
                producer.send(configData.getShortenRequestTopicName(), avroModel.getId(), avroModel);
            }).then();
    }
}
